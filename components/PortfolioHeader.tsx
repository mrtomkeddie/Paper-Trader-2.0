...
import { ArrowUpRight, ArrowDownRight, TrendingUp, TrendingDown, Target, Zap, Clock } from 'lucide-react';
    ...
    <div className="mb-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className={`text-4xl font-black tabular-nums tracking-tighter ${totalDayPnL >= 0 ? 'text-ios-green' : 'text-ios-red'}`}>
        ...
        <div className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-bold ${isPositiveDay ? 'bg-ios-green/15 text-ios-green' : 'bg-ios-red/15 text-ios-red'}`}>
          ...
          <span>{formatCurrency(Math.abs(totalDayPnL || 0))} ({formatNumber(((totalDayPnL || 0) / Math.max(1, totalBalance || 1) * 100), 2)}%)</span>
