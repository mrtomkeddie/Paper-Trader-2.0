...
import { Filter, Search, ChevronDown, CheckCircle2, Terminal, ChevronLeft, ChevronRight, ArrowDownCircle } from 'lucide-react';
...
export const TradeHistory: React.FC<TradeHistoryProps> = ({ trades }) => {
  ...
  useEffect(() => {
    setVisibleCount(TRADES_PER_PAGE);
  }, [activeAgents, selectedDate]);
    ...
    <GlassCard className="h-full flex flex-col p-0 overflow-hidden">
      ...
      <div className="p-4 border-b border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-black/20">
        <h2 className="text-xs font-bold text-premium-cyan/80 uppercase tracking-widest flex items-center gap-2 font-mono">
          <Terminal className="w-4 h-4 text-premium-cyan" />
          Trade_History_Log
        </h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-black/10">
        {visibleTrades.length === 0 ? (
          ...
          <>
            ...
            {hasMore && (
              <div className="p-4 flex justify-center border-t border-white/5 bg-black/20">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleShowMore}
                  rightIcon={<ArrowDownCircle size={14} />}
                  className="text-xs uppercase tracking-widest text-gray-500 hover:text-premium-cyan"
                >
                  Show More
                </Button>
              </div>
