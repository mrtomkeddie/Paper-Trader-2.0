...
function processTicks(symbol) {
  ...
  if (false && asset.activeStrategies.includes('TREND_FOLLOW')) {
    ...
    // D. AGENTIC AI (Manager.js)
    // Replaced legacy logic with multi-agent system
    if (asset.activeStrategies.includes('AI_AGENT')) {
      // Construct enriched market data context
      const marketData = {
        ...asset,
        candles: candlesM5[symbol] || [],
        candlesM15: candlesM15[symbol] || []
      };

      // Tick the Manager (which ticks all agents)
      manager.onTick(symbol, marketData);

      // Execute any new trades generated by agents
      const newAgentTrades = manager.consumeNewTrades();
      for (const t of newAgentTrades) {
        if (t.symbol !== symbol) continue; // Safety check

        // Map Agent decision to Bot execution
        executeTrade(
          symbol,
          t.action || t.type, // 'BUY' or 'SELL'
          asset.currentPrice,
          'AI_AGENT', // Strategy Tag
          'AGGRESSIVE', // Risk Profile (Agents manage their own logical risk, we just execute)
          `${t.agentId ? t.agentId.toUpperCase() : 'AGENT'}: ${t.reason}`,
          t.confidence || 0,
          t.stopLoss
        );
      }
